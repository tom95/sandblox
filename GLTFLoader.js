...
function ServerFileLoader () {
  this.responseType = null
}
ServerFileLoader.prototype.load = function (url, cb) {
  let dataUrl = url.match(/^data:(.*?)(;base64)?,(.*)$/)
  if (dataUrl) {
    cb((new Uint8Array(Buffer.from(dataUrl[3], 'base64'))).buffer)
  } else {
    fs.readFile(url).then(data => cb(data.toString('utf8')))
  }
}
ServerFileLoader.prototype.setResponseType = function (type) {
  this.responseType = type
}
THREE.FileLoader = ServerFileLoader

module.exports = THREE.GLTFLoader = (function () {
